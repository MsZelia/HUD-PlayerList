   ...
   public dynamic class QuickContainerItem extends BSUIComponent
   {
      ...
      override public function redrawUIComponent() : void
      {
         ...
         if(this.data)
         {
            ...
            _loc2_ = Boolean(BSUIDataManager.GetDataFromClient("HUDModeData").data.skipLevelRequirements);
            ...
            {
               if(_loc3_[_loc6_].clip != null && Boolean(_loc3_[_loc6_].visible))
               {
                  _loc4_ += _loc3_[_loc6_].clip.width + 2;
               }
               _loc6_++;
            }
            if(this.data.maximumHealth > 0)
            {
               _loc4_ += this.ConditionMeter_mc.width;
            }
      ...
      public function AddIconsToEntry(param1:Array) : *
      {
         ...
         var _loc2_:* = this.ItemName_tf.getLineMetrics(0).width + this.ItemName_tf.getLineMetrics(0).x + this.ItemName_tf.x + 2;
         var _loc3_:* = 0;
         var _loc5_:* = 0;
         while(_loc5_ < param1.length)
         {
            if((_loc4_ = param1[_loc5_]).clip != null)
            {
               _loc4_.clip.visible = _loc4_.visible;
               if(_loc4_.clip.visible)
               {
                  this.selectedColorTransform(_loc4_.clip,this.selected);
                  _loc4_.clip.x = _loc2_ + _loc3_;
                  _loc3_ += _loc4_.clip.width + 2;
               }
            }
            _loc5_++;
         }
