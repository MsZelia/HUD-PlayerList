   ...
   public class PropertyTint extends EazeSpecial
   {
      ...
      public function PropertyTint(param1:Object, param2:*, param3:*, param4:EazeSpecial)
      {
         ...
         else
         {
            ...
            _loc6_ = 0;
            _loc7_ = 0;
            if((_loc8_ = param3 is Array ? param3 : [param3])[0] === null)
            {
               _loc5_ = 0;
               _loc6_ = 1;
            }
            else
            {
               if(_loc8_.length > 1)
               {
                  _loc5_ = Number(_loc8_[1]);
               }
               if(_loc8_.length > 2)
               {
                  _loc6_ = Number(_loc8_[2]);
               }
               else
               {
                  _loc6_ = 1 - _loc5_;
               }
               _loc7_ = uint(_loc8_[0]);
            }
            this.tvalue = new ColorTransform();
            this.tvalue.redMultiplier = _loc6_;
            this.tvalue.greenMultiplier = _loc6_;
            this.tvalue.blueMultiplier = _loc6_;
            this.tvalue.redOffset = _loc5_ * (_loc7_ >> 16 & 0xFF);
            this.tvalue.greenOffset = _loc5_ * (_loc7_ >> 8 & 0xFF);
            this.tvalue.blueOffset = _loc5_ * (_loc7_ & 0xFF);
         }
      }
